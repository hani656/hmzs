  ...
  <div class="add-role">
    ...
    <footer class="add-footer">
      ...
      <div class="btn-container">
        ...
        <el-button v-if="currentActive===3" type="primary" @click="confirmAdd">{{ `${currentActive===3 && !roleId?'确认添加':'确认修改'}` }}</el-button>
        <!-- <el-button v-if="roleId && currentActive===3" type="primary" @click="updateRole">确认修改</el-button> -->
      </div>
    </footer>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      treeList: [], // tree列表
      disabledTreeList: [], // 回填显示的树列表
      permsList: [] // 存入选中项节点(菜单/功能点权限信息)
    }
  },
  computed: {
    roleId() {
  ...
  methods: {
    ...
    async confirmAdd() {
      ...
      this.$message.success(`${this.roleId ? '修改角色成功' : '添加角色成功'}`)
      this.$router.back()
    },
    async getTreeList() {
      const res = await getTreeListAPI()
      this.treeList = res.data
      // 在disabledTreeList中放一份数据 因为数组是引用类型 在进行赋值的时候实际上赋值是数据引用
      // 也就是说这两份数据实际指向的是同一个内存区域（堆内存）一个修改 另一个也会受到影响
      // 解决办法：深拷贝
    ...
    next() {
      ...
      // 在角色信息这个状态下 做表单验证 如果校验通过 才能进入到下一个状态（this.currentActive++）
      if (this.currentActive === 1) {
        ...
        if (this.permsList.flat().length === 0) {
          ...
          this.currentActive++
          // 回填树形结构
          this.$refs.disabledTree.forEach((treeInstance, index) => {
