...
<script>
import { getRoleListAPI, getTreeListAPI, getRoleDetailAPI, getRoleUserAPI, delRoleAPI } from '@/api/system'

// 递归添加disabled属性
// 重复的执行同一个函数 目的是为了针对每一层中的每一个对象都会执行相同的逻辑
// 相同的逻辑：遍历数组 给每一项搜添加一个属性disable
...
export default {
  ...
  async mounted() {
    ...
    // 1. this.roleList不能是空
    // 2. treeList也不能为空
    // 意味着需要等上面的两个方法中的异步请求返回之后才去调用switchTab方法
    this.switchTab(0)
  },
  methods: {
    // 删除角色
